...
2. Create two files to push up to github.
    ...
    `[root@controller myopenstack]#` `echo "Hello version controlled world!" > hello`

    `[root@controller myopenstack]#` `git add hello`
    
    `[root@controller myopenstack]#` `git status`

    `[root@controller myopenstack]#` `git commit -m 'This is supposed to add my history and hello to my repository'`
