...
10. Display the contents of **keystonerc_chestercopperpot** and verify that they are correct, then check the environmental variables to see the results.

    `[root@controller ~(keystone_admin)#` `cat keystonerc_chestercopperpot`

    `[root@controller ~(keystone_admin)#` `printenv | grep OS_`
