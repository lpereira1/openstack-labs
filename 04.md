...
2. Display a list of the environmental variables currently set
   ...
   You might also wish to issue the following command (which is easier to read):
...
4. Once again, display a list of the environmental variables currently set.
    ...
    You might also try to issue the following command (which may make it easier to read):
    
    `[root@controller ~(keystone_admin)]#` `printenv | sort`
