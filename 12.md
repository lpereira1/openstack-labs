...
0. Of course, edgar being **restricted_user** won't allow him to do **anything**, until we edit any of the /etc/\<service\>/policy.json files so that the role restricted_user is defined as being allowed to perform some actions. To drive this point home, log out of Horizon (if you're signed in) or open a connection to Firefox in the remote desktop (http://controller/horizon).

0. Try to log in to Horizon as **default:edgar:fa5tpa55w0rd**. It won't work.

0. Hopefully it is clear why the user edgar can't do anything. He has a role applied, but that role has not been defined with any allowable actions in the policy.json files. We're not actually going to do that, but hopefully how roles work are clear. If that isn't the case, now is a great time to ask the instructor to explain roles a bit further.
