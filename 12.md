...
0. A policy.json files exists for every service, but the student user does not have sufficent permissions to see them. That's okay, we have plenty to review here! Now let's take a look at what one of these files contains.
