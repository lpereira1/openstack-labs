...
10. Before we finish in Horizon, make sure your VMs are active and running (powered on). Navigate to **Project > Compute > Instance**, and then click the **Start Instace** beside any VMs that are currently listed as Shutoff and Shut down (powered off). If the VMs are not powered off, but instead suspended, then you will need to select **Resume Instance** from the **Actions** dropdown menu.
...
10. In the next section, you will SSH into vt2. Take a moment and make sure vt2 is powered on (and not shutoff). You can see which VMs are powered up, or down, by using the **nova list** command. If needed, vt2 can be started with the **nova start** command.
