...
0.  Keystone's job is management of users and credentials. Issue this command to the keystone service to create a new tenant.
    ...
    `[student@controller ~(keystone_admin)]#` `openstack project create --domain default vault_tek`
...
0. Create the user chestercopperpot, within the domain default, in the project (tenant) vault_tek, with the password fa5tpa55w0rd, and the email address chester@vaulttek.example.
    ...
    `[student@controller ~(keystone_admin)]#` `openstack user create --project vault_tek --password fa5tpa55w0rd --email chester@vaulttek.example --domain default chestercopperpot`
...
0. Add a role to Chestercopperpot
    ...
    `[student@controller ~(keystone_admin)]#` `openstack role add --project vault_tek --user chestercopperpot user`

0. Display the tenants (projects) currently available. 

    `[student@controller ~(keystone_admin)]#` `openstack project list`
...
0. Display the users that have been created in the system. 
    ...
    `[student@controller ~(keystone_admin)]#` `openstack user list`
