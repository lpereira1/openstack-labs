...
## Lab XY - Creating new (public) flavors at the CLI  - &#x2B50;REQUIRED&#x2B50;
...
0. Switch to the user Alice Anderson, and confirm she can see the flavor. Type the following:

    `root@controller admin >` `source alice.rc`

    `root@controller alice >` `openstack flavor list`

0. Switch to the user Chester Copperpot, and confirm he can see the flavor. Type the following: 
...
0. If you haven't done so before, issue the openstack flavor create --help command.
...
0. Notice that a flavor may also have an argument that sets the flavor to private. Let's become admin, then create a new flavor as private, and see what happens.
...
0. Give Alice's project (Acme Inc) access to the flavor as well. Discover the project ID of Acme Inc. 
	...
	`root@controller admin >` `openstack project list` <-- Discover Acme Inc's project ID  
...
0. That's it for this lab! We'll continue to explore manipulating instances, but feel free to exploring launching instances from the CLI or Horizon, or checking out other Nova commands while you wait.
