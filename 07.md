 ...
 * If the instance is not ACTIVE and Running, let the instructor know
    ...
    > An interesting option on step three is to try this:  
    >   ``VT1=`nova list --all-tenants | grep vt1 | cut -d ' ' -f 7` ``
    > As a result, this will sote the Tenant ID in $VT1.  Here how it works:  
    > VT1 is a variable name  
    > The backtick **`** surrounds the entire nova command to cause the results to be placed oin the variable $VT1  
    > `nova list --all-tenants` will list all running VMs in openstack
    > `| grep vt1` will filter out all VMs not named **vt1**  
    > `cut -d ' '` set the delimiter characted to a space  
    > `-f 7` means to find the seventh feild surrounded by spaces  
