...
Back to our story. Time to take a second order, only this time we'll complete the order with the use of the CLI.
   ...
   - Order 2: Vault Tek; for now they would like one user, Chester Copperpot, to be able to admin a small cloud for their company.

Remember, we still promised our customer Vault Tek Enterprise that they could have access to a project space with a single user, Chester Copperpot. Let's try setting this user up at the CLI.
