...
0. So it doesn't look like any environmental variables containing OS have been set, but try that command against. This time, pass a grep command to say you're specifically searching for any environmental variables that begin with OS_
...
0. Assocaite the tenant name with the UUID of Chester's default_project_id with the id below (it should be vault_tek).
    ...
    ---> | 6417af4dea224c5999e89a7634d27645 | vault_tek     | <-- vault_tek should match Chester's <DEFAULT_PROJECT_ID> from above!
         | 712e8501dd204df188c64f44bd81093d | service       |
         +----------------------------------+---------------+
    ```

0. Often times the python-clients require using UUIDs. By design, these are unique on every system, so you will need to occasionally customize a command. In the next command, you need to replace \*\*\<DEFAULT_PROJECT_ID\>\*\* with the value you retrieved in the last step. 

    `student@controller (admin) :~$`  `openstack project show **<DEFAULT_PROJECT_ID>**`
    > Just as an example, the value the lab examples would use is *openstack project show 6417af4dea224c5999e89a7634d27645*
