...
The objective of this lab is to verify that all OpenStack services are correctly installed and running.

### Procedure

0. To start this lab, you'll need to have a terminal open, and have an SSH session open to controller. You may already have this session open, if so you can use that session, if not, type the following.

    `[student@beachhead]#` `ssh controller`

0. If you've successfully started an ssh session with the controller, your prompt will have changed to read `student@controller`. Return to the student home directory.

    `student@controller:~$` `cd`


0. Get access to the admin user to make changes to verify Openstack. Again, don't worry, we'll go deeper into this command a bit later.
    ...
    `[ender@controller]#` `source admin.rc`  =  `[ender@controller]#` `. admin.rc`
     > Even if you've run this command previously, it won't hurt to run this command a second time.
